...
class DB_Mysql():
	...
	self.dbc.execute("LOCK TABLES pool_worker write, shares write, pool write");
	self.dbc.execute("update pool_worker set speed = 0, alive = 0");
#        stime =  float( time.time() - averageOverTime );
	stime = '%.0f' % ( time.time() - averageOverTime );
# 	self.dbc.execute("select username,ABS(SUM(difficulty)) from shares where time > FROM_UNIXTIME(%s) group by username", (stime,))
        self.dbc.execute("select username,count(id) from shares where time > FROM_UNIXTIME(%s) group by username", (stime,))


	total_speed = 0
	for name,shares in self.dbc.fetchall():
	    speed = Decimal(Decimal(Decimal(shares) * Decimal(4294967296)) // ( Decimal(averageOverTime) * 1000 * 1000))

#	    speed = float(float(shares) * float(4294967296)) / ( float(averageOverTime) )
#        log.debug("%s"), (speed)
	    total_speed += speed
	    self.dbc.execute("update pool_worker set speed = %s, alive = 1 where username = %s", (Decimal(speed),name))
	...
	# Check for found shares to archive
        self.dbc.execute("LOCK TABLES shares write");
	self.dbc.execute("select time from shares where upstream_result = 'Y' order by time limit 1")
        self.dbc.execute("UNLOCK TABLES");
...
#	data: [worker_name,block_header,block_hash,difficulty,timestamp,is_valid,ip,block_height,prev_hash,invalid_reason,best_diff]
	...
	for k,v in enumerate(data):
		...
		if v[10] > best_diff:
	        ...
	        self.dbc.execute("LOCK TABLE shares write")
		self.dbc.execute("insert into shares " +\
			...
			(time.time(),v[6],v[0],v[5],0,v[9],v[2],v[7],v[8],'',v[3]) )
	    else :
                total_shares += v[3]
                if v[0] in checkin_times:
                    if v[4] > checkin_times[v[0]] :
                ...
                self.dbc.execute("UNLOCK TABLES")
		self.dbc.execute("LOCK TABLE shares write")
		self.dbc.execute("insert into shares (time,rem_host,username,our_result,reason,solution,upstream_result,difficulty) VALUES " +\
			"(FROM_UNIXTIME(%s),%s,%s,%s,%s,%s,%s,%s)",
			(time.time(),v[6],v[0],'Y','',v[2],v[5],v[3]) )
                self.dbc.execute("UNLOCK TABLES")
	...
	# Note: difficulty = -1 here
        self.dbc.execute("LOCK TABLE shares write, pool_worker write")
	self.dbc.execute("update shares set upstream_result = %s where time = FROM_UNIXTIME(%s) and username = %s limit 1",
		(data[5],data[4],data[0]))
        self.dbc.execute("LOCK TABLE shares write, pool_worker write")
        self.dbc.execute("update shares set solution = %s where time = FROM_UNIXTIME(%s) and username = %s limit 1",
                (data[2],data[4],data[0]))
	...
	log.debug("Deleting Username")
        self.dbc.execute("LOCK TABLE pool_worker write")
    ...
    def update_pool_info(self,pi):
        self.dbc.execute("LOCK TABLE pool write")
	...
	while version < current_version :
	    self.dbc.execute("select value from pool where parameter = 'DB_Version'")
	    data = self.dbc.fetchone()
	    version = int(data[0])
	...
	if settings.DATABASE_EXTEND == True :
	    ...
	    self.dbc.execute("insert into pool (parameter,value) VALUES ('DB_Version',2)")
	...
	self.dbc.execute("update pool set value = 3 where parameter = 'DB_Version'")
	self.dbh.commit()
	...
	self.dbc.execute("update pool set value = 4 where parameter = 'DB_Version'")
	self.dbh.commit()
	...
	self.dbc.execute("create table if not exists shares_archive_found " +\
		"(id serial primary key,time timestamp,rem_host TEXT, username TEXT, our_result BOOLEAN, upstream_result BOOLEAN, reason TEXT, solution TEXT, " +\
		"block_num INTEGER, prev_block_hash TEXT, useragent TEXT, difficulty INTEGER) ENGINE = MYISAM;")
	self.dbc.execute("update pool set value = 5 where parameter = 'DB_Version'")
	self.dbh.commit()
	...
	self.dbc.execute("update pool set value = 6 where parameter = 'DB_Version'")
	self.dbh.commit()
